...
INACTIVE_DAYS = 30              # Дней до статуса "неактивен"
DATA_FILE = "bot_data.json"     # Файл базы данных

# Антиспам
SPAM_WINDOW = 120               # Окно времени (секунды)
SPAM_LIMIT = 5                  # Лимит одинаковых сообщений
SPAM_MUTE_DURATION = 600        # Длительность мута (секунды)

# Репутация
REP_COOLDOWN = 60               # Кулдаун репутации (секунды)

# Роли для "!кто я"
WHOIS_ROLES = [
    "🎮 Геймер", "☕ Кофеман", "🌙 Сова", "📚 Книжный червь",
    "🎵 Меломан", "🍕 Пицца-любитель", "😴 Соня", "🚀 Мечтатель",
    "🎭 Актер", "💻 Кодер", "🎨 Художник", "🐾 Любитель котов",
    "🏋️ Спортсмен", "🌍 Путешественник", "👾 Гик", "🌟 Звезда чата"
]
...
def get_warns(chat_id, user_id):
    return data["warnings"].get(f"{chat_id}_{user_id}", 0)

def add_warn(chat_id, user_id):
    k = f"{chat_id}_{user_id}"
    data["warnings"][k] = data["warnings"].get(k, 0) + 1
    save_data()
    return data["warnings"][k]

def reset_warns(chat_id, user_id):
    k = f"{chat_id}_{user_id}"
    if k in data["warnings"]:
        del data["warnings"][k]
        save_data()

def get_rep(chat_id, user_id):
    return data["reputation"].get(f"{chat_id}_{user_id}", 0)

def change_rep(chat_id, user_id, delta):
    k = f"{chat_id}_{user_id}"
    data["reputation"][k] = data["reputation"].get(k, 0) + delta
    save_data()
    return data["reputation"][k]

def set_nickname(chat_id, user_id, nick):
    data["nicknames"][f"{chat_id}_{user_id}"] = nick.strip()
    save_data()

def del_nickname(chat_id, user_id):
    k = f"{chat_id}_{user_id}"
    if k in data["nicknames"]:
        del data["nicknames"][k]
        save_data()
...
def increment_msg_count(chat_id, user_id):
    k = f"{chat_id}_{user_id}"
    now = int(time.time())
    if k not in data["message_stats"]:
        data["message_stats"][k] = {"count": 0, "last_active": now}
    data["message_stats"][k]["count"] += 1
    data["message_stats"][k]["last_active"] = now
    if k not in data["user_joins"]:
        data["user_joins"][k] = now
    save_data()
...
async def has_restrict_rights(bot: Bot, chat_id: int, user_id: int) -> bool:
    """
    Проверяет права на ограничение участников.
    - Создатель (creator) имеет ВСЕ права по умолчанию
    - Администратор должен иметь галочку "Ограничение участников"
    """
    try:
        member = await bot.get_chat_member(chat_id, user_id)
...
def parse_duration(duration_str: str) -> int:
    """Парсинг длительности (1h, 30m, 1d)"""
    if not duration_str:
...
async def cmd_help(message: Message):
    await message.answer(
        ...
        "<code>+чат / -чат</code> - открыть чат / закрыть чат\n\n"
        "<b>Все:</b>\n"
        "<code>+ник имя / -ник</code> - установить ник / удалить\n"
        "<code>+реп / -реп</code> - репутация (ответом)\n"
        "<code>!репорт причина</code> - жалоба (ответом)\n"
        "<code>!топ [реп|соо|дней]</code> - топы\n"
        "<code>!кто я / !кто ты</code> - случайная роль\n"
        "<code>!неактив</code> - неактивные\n"
        "<code>!админы</code> - призыв админов\n"
        "<code>!внимание</code> - объявление\n\n"
        "🛡️ Антиспам: 5+ одинаковых = мут 10 мин",
        parse_mode="HTML"
    )
...
async def cmd_chat(message: Message):
    ...
    try:
        if is_open:
            ...
            await message.answer("🔓 <b>Чат открыт!</b>", parse_mode="HTML")
        else:
            await message.bot.set_chat_permissions(
                message.chat.id,
                ChatPermissions(
            ...
            await message.answer("🔒 <b>Чат закрыт!</b>", parse_mode="HTML")
    except Exception as e:
...
async def cmd_report(message: Message):
    ...
    if violator.is_bot:
        return await message.answer("❌ Нельзя жаловаться на ботов.")
    ...
    if ADMIN_CHAT_ID and ADMIN_CHAT_ID != "":
        try:
            ...
            logging.error(f"Ошибка репорта: {e}")
...
async def cmd_nick(message: Message, command: CommandObject):
    ...
    if is_add:
        ...
        if not nick:
            return await message.answer("❌ Укажите ник: <code>+ник Имя</code>", parse_mode="HTML")
...
async def cmd_ban(message: Message, command: CommandObject):
    ...
    if target.is_bot:
        return await message.answer("❌ Нельзя банить ботов")
    
    if await is_group_admin(message.bot, message.chat.id, target.id):
        return await message.answer("⛔ Нельзя банить админов")
...
async def cmd_mute(message: Message, command: CommandObject):
    ...
    if target.is_bot:
        return await message.answer("❌ Нельзя мутить ботов")
    
    if await is_group_admin(message.bot, message.chat.id, target.id):
        return await message.answer("⛔ Нельзя мутить админов")
    ...
    if message.text.strip().startswith("+"):
        ...
        try:
            await message.bot.restrict_chat_member(
                message.chat.id, target.id, ChatPermissions(can_send_messages=False), until_date=until
            )
        ...
        full = ChatPermissions(
            can_send_messages=True, can_send_polls=True,
            can_add_web_page_previews=True, can_send_other_messages=True, can_invite_users=True
        )
...
async def cmd_warn(message: Message, command: CommandObject):
    ...
    if target.is_bot:
        return await message.answer("❌ Нельзя варнить ботов")
    
    if await is_group_admin(message.bot, message.chat.id, target.id):
        return await message.answer("⛔ Нельзя варнить админов")
...
async def cmd_rep(message: Message, command: CommandObject):
    ...
    if target.is_bot:
        return await message.answer("❌ Только людям")
    ...
    for k in list(rep_cooldowns.keys()):
        if now - rep_cooldowns[k] >= REP_COOLDOWN:
            del rep_cooldowns[k]
    ...
    await message.answer(
        f"{'👍 +1' if delta > 0 else '👎 -1'} {get_display_name(message.chat.id, target.id, target.full_name)}: {new_rep}"
    )
...
async def cmd_who(message: Message):
    ...
    await message.answer(
        f"🔮 <b>{get_display_name(message.chat.id, target.id, target.full_name)}</b> — это {role}!\n📊 Уверенность: {confidence}%",
        parse_mode="HTML"
    )
...
async def cmd_inactive(message: Message):
    ...
    inactive = [
        (int(k.split("_")[1]), now - v["last_active"])
        for k, v in data["message_stats"].items()
        if k.startswith(prefix) and (now - v["last_active"]) // 86400 >= INACTIVE_DAYS
    ]
    inactive.sort(key=lambda x: x[1], reverse=True)
    
    if not inactive:
        return await message.answer(f"✅ Все активны за {INACTIVE_DAYS} дней!")
...
async def cmd_admins(message: Message):
    try:
        ...
        mentions = [
            f'<a href="tg://user?id={a.user.id}">{a.user.full_name}</a>'
            for a in admins if not a.user.is_bot
        ]
        
        if not mentions:
            return await message.answer("❌ Нет админов")
...
async def on_join(message: Message):
    for user in message.new_chat_members:
        if not user.is_bot:
            increment_msg_count(message.chat.id, user.id)
